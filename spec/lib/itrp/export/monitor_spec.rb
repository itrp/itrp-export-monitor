...
describe Itrp::Export::Monitor do
  ...
  context 'initialize' do
    it 'should concatenate the :ids and :id options' do
      Itrp::Export.configuration.id = 3
      monitor = Itrp::Export::Monitor.new
    ...
    [:name, :root, :ids, :imap_user_name, :imap_password].each do |required_option|
      it "should raise an exception is the required option #{required_option} is missing" do
        Itrp::Export.configuration.send(:"#{required_option}=", required_option == :ids ? [] : '')
        expect{ Itrp::Export::Monitor.new }.to raise_error(::Itrp::Exception, "Missing required configuration option #{required_option}")
      end
    end

    it 'should set the logger' do
      logger = Logger.new($stdout)
      Itrp::Export.configuration.logger = logger
      monitor = Itrp::Export::Monitor.new
  ...
  context 'process' do
    ...
    it 'should skip non-monitored export ids' do
      Itrp::Export.configuration.ids = [1,3]
      monitor = Itrp::Export::Monitor.new
    ...
    context 'store_export' do
      before(:each) do
        ...
        @local_filename = "#{Itrp::Export.configuration.root}/downloads/20130911-195545-affected_slas.csv"
      end
      ...
      it 'should copy the export file to another location' do
        expect(@monitor).to receive(:ftp_export)
        expect_log("Processing ITRP Export mail:\n  Subject: Export finished - Full ad hoc export -- ITRP example\n  Export ID: 2\n  Token: #{@export_token}\n  URI: #{@export_uri}")
        expect_log("Copied export '#{@local_filename}' to '#{Itrp::Export.configuration.to}'")

        @monitor.process(@export_mail)

        File.read(@local_filename).should == 'exported content'
        File.read("#{Itrp::Export.configuration.to}/20130911-195545-affected_slas.csv").should == 'exported content'
      end

      it 'should FTP the export file' do
        expect(@monitor).to receive(:copy_export)
        expect_log("Processing ITRP Export mail:\n  Subject: Export finished - Full ad hoc export -- ITRP example\n  Export ID: 2\n  Token: #{@export_token}\n  URI: #{@export_uri}")
        expect_log("FTP export '#{@local_filename}' to '#{Itrp::Export.configuration.to_ftp}/20130911-195545-affected_slas.csv'")
  ...
  it 'should create a clacks config file' do
    clacks_filename = Itrp::Export::Monitor.new.generate_clacks_config
...
Clacks.config.pid = "/home/mathijs/dev/itrp-export-monitor/spec/tmp/exports/pids/export_monitor.1.2.pid"
Clacks.config.stdout_path = "/home/mathijs/dev/itrp-export-monitor/spec/tmp/exports/logs/export_monitor.1.2.log"
Clacks.config.stderr_path = "/home/mathijs/dev/itrp-export-monitor/spec/tmp/exports/logs/export_monitor.1.2.log"
